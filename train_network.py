...
def forward(
    ...
    with torch.set_grad_enabled(is_train):
        for tensor, labels in loader:
            ...
            for head, head_logits in heads.items():
                ...
                for d, t, p in zip(
                    domain.tolist(), head_labels.tolist(), head_preds.tolist()
                ):
    ...
    for head, head_weight in config.heads.items():
        head_rvce = head_weight * np.mean(
            [
                abs(n_events_true[head][domain] - n_events_pred[head][domain])
                / n_events_true[head][domain]
    ...
    mae = np.mean(
        [
            ...
            for head, head_abs_errors in abs_errors.items()
        ]
    )
...
def run(config):
    ...
    trn_dataset = VehicleDataset(
        trn_datapool, part=trn_part, config=config, is_trn=True
    )
